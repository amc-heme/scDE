  ...
  function(
    ...
    if (inherits(object[[seurat_assay]], "Assay5")) {
      ...
      if (layer == "counts") {
        ...
        stop(
          paste0(
            'The assay passed to the object (',
            seurat_assay,
            ') is a Seurat ',
...
run_dge.SingleCellExperiment <-
  function(
    ...
    .make_sentinel <- function(group_levels) {
      sentinel <- ".scDE_other"
      while (sentinel %in% group_levels) {
    ...
    .is_presto_compatible <- function(mat) {
      inherits(mat, c("dgCMatrix", "matrix"))
    }
    ...
    # MatrixGenerics::rowMeans2 (called below) dispatches on matrix class.
    # Sparse and DelayedArray-backed matrices need a backend package to
    # provide the method; without it MatrixGenerics raises an opaque
    # "Failed to find a rowMeans2() method" error.
    if (test_use == "scran") {
      if (!requireNamespace("scran", quietly = TRUE)) {
        stop(
          "Package 'scran' is required when test_use is '",
          test_use,
          "'. ",
      ...
      if (
        ...
        stop(
          "Package 'DelayedMatrixStats' is required for DelayedArray-backed ",
          "assays when scran is used for run_dge. ",
          "Install with: BiocManager::install('DelayedMatrixStats')"
        )
      }
      if (
        inherits(mat, "dgCMatrix") &&
          !requireNamespace("sparseMatrixStats", quietly = TRUE)
      ) {
        stop(
          "Package 'sparseMatrixStats' is required for sparse assays when ",
          "scran is used for run_dge. ",
          "Install with: BiocManager::install('sparseMatrixStats'), ",
          "or alternatively set test_use = 'Presto' in run_dge()."
        )
      }
    }
